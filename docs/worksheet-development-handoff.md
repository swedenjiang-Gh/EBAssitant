...
- 第一行是列标签，EB2023 下创建后会写入工作表内部列标签记录。
- 第二行是属性 ID。
- 工作表对象类型当前固定为“器件”。
...
当前正式功能在 EB2023 中写入自定义列标签。

实现路线：

1. 先通过 COM 创建工作表、添加属性列、设置列宽，并保存到 `工作表 / 收藏夹`。
2. 保存后从收藏夹读回唯一工作表配置对象。
3. 将配置对象 ID 转换为数据库 OID。
4. 查询 `dbo.[Object]` 中 `OIDParent = 配置OID AND CID = 19` 的内部列记录。
5. 按 `OrderNumber, OID` 排序，将 Excel 第一行标签逐列映射到这些记录。
6. 开启 SQL 事务，按 exact `OID + OIDParent + CID=19` 更新 `Designation`。
7. 每条更新后使用 `SELECT @@ROWCOUNT`，必须等于 1。
8. 在事务内读回确认标签值一致后提交。
...
- 该路线当前只允许 EB2023。
- 不按工作表名称直接写入。
- 不写任何未由本次保存并读回确认的工作表配置对象。
- 不调用 `aucCmdEditColumnLabel`，不做 UI Automation。
- 不使用 `Worksheet.Columns`，因为既有样本中该集合可能读不到内部列记录。
- `ExecuteNonQuery()` 返回值不作为命中行数依据，因为 EB 表触发器可能放大影响行数。
...
- 单个工作表创建、加列、设宽、保存、列标签写入或读回失败：该工作表记录为 `failed`，继续处理后续工作表。
- 创建成功：记录为 `created`。
...
6. 将工作表配置保存到该项目的 `工作表 / 收藏夹`。
7. 使用受控内部列记录路线，把 Excel 第一行写入该工作表配置的列标签。
...
- 若后续要支持 EB2024 或其他对象类型列标签写入，必须重新验证对应版本和对象类型的内部列记录结构，不能直接套用 EB2023 结果。
