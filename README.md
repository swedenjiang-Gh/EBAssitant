...
| 入口 | 已实现内容 |
| --- | --- |
| 属性 | 读取并缓存属性目录树；新建属性目录；从 Excel 批量创建属性；支持属性类型映射；结果写入本地日志。 |
| 类型定义 | 读取并缓存类型定义树；父子复选联动；从 Excel 批量把 AID 添加到叶级 TypeItem 的定义对话框选项卡；结果写入本地日志。 |
| 工作表 | 读取并缓存项目模板树；在模板项目中从 Excel 批量创建器件工作表；保存到所选项目自己的 `工作表 / 收藏夹`；结果写入本地日志。 |
...
- 工作表列标签只用于预览、列宽计算和日志；正式创建不写入自定义列标签。
